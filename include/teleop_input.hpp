...
// Replays a previously recorded sequence of frames. In reverse mode, frames
// are walked from last to first with gear flipped drive<->reverse (steering
// is kept as-recorded — see README/plan for why: replaying the same wheel
// position in reverse gear is what actually retraces a drive back out,
// unlike negating steering). This is an approximation of a real reverse
// maneuver, not an exact inverse of the vehicle's dynamics.
