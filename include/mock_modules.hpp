...
// each tick, so callers don't care whether it's synthesized from the route
// (MockPlanner) or pulled from an authored trajectory
// (FixedTrajectorySource).
...
// vehicle's current progress along it. Used instead of MockPlanner whenever
// scenario.trajectory is non-empty — see sim_loop.cpp. Points are used
// exactly as authored (curved segments expanded along their Bezier, straight
// ones passed through untouched) — what you drew is what ControlManager sees.
